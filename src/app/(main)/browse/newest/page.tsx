  ...
  return (
    <div style={{ maxWidth: '430px', margin: '0 auto', paddingBottom: '6rem' }}>
      <div style={{ padding: '1.25rem 1rem 0.5rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-fraunces), Georgia, serif', fontSize: '24px' }}>Curatorial picks</h1>
      </div>
      <div style={{ padding: '4px 1rem 12px', color: '#999', fontSize: '13px' }}>
        {usingFallback ? 'Our selection is coming soon — newest works for now' : `${artworks.length} selected works`}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', padding: '0 1rem' }}>
        {artworks.map(a => <ArtworkCard key={a.id} artwork={a} />)}
      </div>
      {artworks.length === 0 && <div style={{ padding: '3rem', textAlign: 'center', color: '#999' }}>No works yet.</div>}
    </div>
  )
}