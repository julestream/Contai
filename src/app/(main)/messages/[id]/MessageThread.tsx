...
// The chat has to fit between the app's top bar and its tab bar. The old
// value reserved 56px for both, so the composer sat below the fold and the
// artwork header scrolled out of view. These are the real heights.
const TOP_BAR = 58
const TAB_BAR = 74

export default function MessageThread({ conversation, initialMessages, initialOffers, currentUserId, backLabel }: any) {
  ...
  return (
    <div style={{
      ...
      height: `calc(100dvh - ${TOP_BAR + TAB_BAR}px)`,
      display: 'flex', flexDirection: 'column', background: '#fff',
    }}>
      {/* Header — back link and artwork on one row, so the piece being
          discussed stays in view while typing. */}
      <div style={{
        padding: '10px 12px', borderBottom: '1px solid #e8e8e8',
        display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0,
      }}>
        <Link href="/messages" aria-label={backLabel} style={{ color: '#8a857c', display: 'flex', flexShrink: 0, textDecoration: 'none' }}>
          <ChevronLeft size={20} />
        </Link>
        <Link href={artwork?.id ? `/artwork/${artwork.id}` : '/messages'} style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none', minWidth: 0, flex: 1 }}>
          {images?.length > 0 && (
            <img src={images[0]} style={{ width: '36px', height: '36px', objectFit: 'cover', borderRadius: '6px', flexShrink: 0 }} />
          )}
          <div style={{ minWidth: 0 }}>
            <p style={{ fontWeight: 600, fontSize: '14px', color: '#0a0a0a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {artwork?.title}
            </p>
      ...
      {/* Timeline */}
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {timeline.map((item) => {
      ...
      {/* Safety notice */}
      {showSafety && (
        <div style={{ flexShrink: 0, padding: '8px 12px', background: '#fbf3e2', borderTop: '1px solid #f0e2c4', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
          <p style={{ fontSize: '11.5px', color: '#7a5d1e', lineHeight: 1.4, flex: 1, margin: 0 }}>{t('messages.safety')}</p>
      ...
      {/* Offer composer */}
      {showOfferInput && (
        <div style={{ flexShrink: 0, padding: '10px 12px', borderTop: '1px solid #e8e8e8', display: 'flex', gap: '8px', alignItems: 'center' }}>
          <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: '6px', border: '1px solid #e0dcd3', borderRadius: '999px', padding: '0 14px' }}>
            <input
              ...
              inputMode="numeric"
              style={{ flex: 1, minWidth: 0, padding: '11px 0', border: 'none', fontSize: '15px', outline: 'none', background: 'transparent' }}
            />
          ...
          <button onClick={() => submitOffer(isArtist ? 'artist' : 'buyer')} disabled={working}
            style={{ padding: '11px 16px', borderRadius: '999px', border: 'none', background: '#0a0a0a', color: '#fff', fontSize: '14px', cursor: 'pointer', flexShrink: 0 }}>
            {t('messages.send')}
          </button>
      ...
      <div style={{ flexShrink: 0, padding: '10px 12px', borderTop: '1px solid #e8e8e8', display: 'flex', gap: '8px', alignItems: 'center' }}>
        ...
        <input
          ...
          style={{ flex: 1, minWidth: 0, padding: '11px 14px', borderRadius: '999px', border: '1px solid #e0dcd3', fontSize: '15px', outline: 'none' }}
        />
        <button onClick={sendMessage} disabled={sending} aria-label={t('messages.send')}
          style={{ padding: '11px', borderRadius: '999px', border: 'none', background: '#0a0a0a', color: '#fff', cursor: 'pointer', display: 'flex', flexShrink: 0 }}>
          <Send size={18} />
        </button>
      </div>
    </div>
